...
export default function App() {
  const [items, setItems] = useState(() => {
    try { const s = localStorage.getItem("cue_items"); return s ? JSON.parse(s) : SAMPLE_ITEMS; }
  ...
  function deleteItem(id) { setItems(prev => prev.filter(i => i.id !== id)); setView("home"); }
      ...
      {view === "home" && (
        <div className="slide-in">
          <div style={{ padding: "52px 24px 20px" }}>
            {/* Logo — matches the green icon */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 28 }}>
              <div style={{
                background: BRAND.green, color: BRAND.navy,
                fontWeight: 700, fontSize: 17,
                padding: "6px 16px", borderRadius: 10, letterSpacing: 0.3,
              }}>Cue</div>
            ...
            <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 30, fontWeight: 400, color: BRAND.navy, lineHeight: 1.2 }}>
              {overdueCount > 0
                ? <>{overdueCount} item{overdueCount !== 1 ? "s" : ""} <span style={{ fontStyle: "italic", color: urgencyColor.overdue }}>overdue</span></>
                : soonCount > 0
                ? <><span style={{ fontStyle: "italic", color: urgencyColor.soon }}>{soonCount} item{soonCount !== 1 ? "s" : ""}</span> need attention</>
          ...
          <div style={{ display: "flex", gap: 8, padding: "0 24px 20px", overflowX: "auto" }}>
            {["all", ...Object.keys(CATEGORIES)].map(cat => (
              <div key={cat} className="cat-chip" onClick={() => setFilterCat(cat)}
                style={{
                  background: filterCat === cat ? BRAND.navy : BRAND.surface,
                  color:      filterCat === cat ? "#fff" : BRAND.navy,
                  border:     `1.5px solid ${filterCat === cat ? BRAND.navy : BRAND.border}`,
                }}>
      ...
      {view === "add" && (
        <div className="slide-in">
          <div style={{ padding: "52px 24px 24px" }}>
            ...
            <div style={{ marginTop: 8 }}><button className="btn-green" onClick={addItem}>Add reminder</button></div>
          </div>
        </div>
      )}
            ...
            {!editMode ? (
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <button className="btn-green" onClick={() => completeItem(selectedItem)}>
              ...
              <>
                ...
                <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 10 }}>
                  <button className="btn-green" onClick={updateItem}>Save changes</button>
